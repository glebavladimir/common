...
metatests.testSync('RangeIterator with start and end', test => {
  const actual = Iterator.range(1, 5).toArray();
  test.strictSame(actual, [1, 2, 3, 4]);
});

metatests.testSync('RangeIterator with start, end and step', test => {
...
metatests.testSync('RangeIterator empty range with start > end', test => {
